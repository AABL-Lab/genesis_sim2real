...
class GenesisGym(gymnasium.Env):
    ...
    def init_env(self):
        ...
        self.wrist_pos_offset = torch.Tensor([0.0, 0.0, 0.02]).to(device=wrist.get_pos().device)
        self.update_camera_position()

    def update_camera_position(self):
        # Update the camera position based on the end effector position
        wrist = self.kinova.get_joint('joint_6')
        wrist_pos = wrist.get_pos()
        position = wrist.get_pos() + self.wrist_pos_offset

        # get the mean position of the two fingertips
        left_fingertip = self.kinova.get_joint('left_finger_tip_joint')
        right_fingertip = self.kinova.get_joint('right_finger_tip_joint')
        middle = (left_fingertip.get_pos() + right_fingertip.get_pos()) / 2 + self.wrist_pos_offset

        self.cam_0.set_pose(pos=position.cpu().numpy(), lookat=middle.cpu().numpy(), up=(0, 0, 1))

    def step(self, action):
        # Apply 
        # the action to the scene
